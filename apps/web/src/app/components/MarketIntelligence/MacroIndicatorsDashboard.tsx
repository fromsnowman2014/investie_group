...
export default function MacroIndicatorsDashboard({ }: MacroIndicatorsDashboardProps) {
  ...
  React.useEffect(() => {
    if (data && process.env.NODE_ENV === 'development') {
      console.log('📊 Macro Indicators Data:', {
        treasury: data.economicIndicators?.interestRate?.value,
        cpi: data.economicIndicators?.cpi?.value,
        fearGreed: data.fearGreedIndex?.value,
        vix: data.vix?.value
      });
    }
  }, [data]);
        ...
        <div className="metrics-section">
          ...
          <div className="metric-chip">
            ...
            <span className="metric-value">
              {data.fearGreedIndex ? data.fearGreedIndex.value.toFixed(0) : '-'}
            </span>
          </div>
          <div className="metric-chip">
            ...
            <span className="metric-value">
              {data.economicIndicators?.interestRate
                ? `${data.economicIndicators.interestRate.value.toFixed(2)}%`
                : '-'}
            </span>
          </div>
