...
export class FearGreedIndexService {
  ...
  constructor() {
    if (!this.alphaVantageApiKey) {
      this.logger.warn('Alpha Vantage API key not configured for Fear & Greed calculation');
    }
  }
  ...
  async calculateFearGreedIndex(): Promise<FearGreedIndex> {
    try {
      ...
      return {
        ...
        confidence,
        components,
        methodology: 'Proprietary calculation using VIX, volume, momentum, breadth, and sentiment indicators',
  ...
  private async getVixData(): Promise<number> {
    try {
      ...
      const vixValue = parseFloat(response.data['Global Quote']['05. price']) || 20;
  ...
  private async getSP500VolumeData(): Promise<number> {
    try {
      ...
      const volume = parseInt(response.data['Global Quote']['06. volume']) || 50000000;
   ...
   */
  private async getMarketBreadthData(): Promise<{ advancing: number; declining: number }> {
    try {
      // Simplified breadth calculation using major sector ETFs
      const sectors = ['XLK', 'XLV', 'XLF', 'XLE', 'XLI', 'XLY', 'XLP', 'XLRE', 'XLB', 'XLU'];
      let advancing = 0;
      let declining = 0;
   ...
   */
  private calculateVolatilityScore(vixData: PromiseSettledResult<number>): number {
  ...
  private calculateVolumeScore(volumeData: PromiseSettledResult<number>): number {
    const volume = volumeData.status === 'fulfilled' ? volumeData.value : 50000000;
  ...
  private calculateBreadthScore(breadthData: PromiseSettledResult<{ advancing: number; declining: number }>): number {
    const breadth = breadthData.status === 'fulfilled' ? breadthData.value : { advancing: 5, declining: 5 };
  ...
  private calculateSafehavenScore(bondYieldData: PromiseSettledResult<number>): number {
    const bondYield = bondYieldData.status === 'fulfilled' ? bondYieldData.value : 4.25;
  ...
  private calculateWeightedFearGreed(components: FearGreedComponents): number {
    const weights = {
      ...
      marketVolume: 0.15,
      marketMomentum: 0.20,        // Momentum is important
      stockPriceBreadth: 0.15,
      safehavenDemand: 0.10,
      junkBondDemand: 0.10,
      putCallRatio: 0.05
    };
    ...
    Object.entries(weights).forEach(([key, weight]) => {
      weightedScore += (components[key as keyof FearGreedComponents] || 50) * weight;
    });
   ...
   */
  private classifyFearGreedStatus(value: number): 'extreme-fear' | 'fear' | 'neutral' | 'greed' | 'extreme-greed' {
