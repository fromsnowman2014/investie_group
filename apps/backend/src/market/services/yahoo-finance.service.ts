...
export class YahooFinanceService {
  ...
  async getQuote(symbol: string): Promise<YahooFinanceQuote | null> {
    try {
      ...
      this.logger.error(`Error fetching Yahoo Finance quote for ${symbol}:`, error.message);
      return null;
    }
  }
   ...
   */
  async getHistoricalData(symbol: string, days: number = 7): Promise<YahooHistoricalData[]> {
    try {
      ...
      this.logger.error(`Error fetching Yahoo Finance historical data for ${symbol}:`, error.message);
      return [];
    }
  }
  ...
  async getSPYSparklineData(): Promise<any> {
    try {
      ...
      const weekAgoPrice = sparklineData.length > 0 ? sparklineData[0].price : currentPrice;
      ...
      this.logger.error('Error creating SPY sparkline data from Yahoo Finance:', error.message);
      throw error;
    }
  }
  ...
  async validateSPYPrice(price: number): Promise<boolean> {
    try {
      ...
      this.logger.warn('Could not validate SPY price against Yahoo Finance:', error.message);
