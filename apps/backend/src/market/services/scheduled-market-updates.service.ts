...
import {
  ...
  CPIData,
  UnemploymentData,
} from './fred-api.service';
import {
  FearGreedIndexService,
  FearGreedIndex,
} from './fear-greed-index.service';
import {
  AlphaVantageService,
  SP500SparklineData,
  EnhancedSectorPerformance,
  VixData,
} from './alpha-vantage.service';
...
export class ScheduledMarketUpdatesService {
  ...
  async marketOpenUpdate(): Promise<UpdateSummary> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Market open update completed successfully in ${summary.duration}ms`,
      );
      return summary;
    } catch (error) {
  ...
  async marketCloseUpdate(): Promise<UpdateSummary> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Market close update completed successfully in ${summary.duration}ms`,
      );
      return summary;
    } catch (error) {
  ...
  async intradayMarketUpdate(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `⚡ Intraday update completed: ${successful}/${results.length} successful`,
      );
    } catch (error) {
   ...
   */
  async executeImmediateUpdate(
    session?: 'market_open' | 'market_close',
  ): Promise<UpdateSummary> {
    const rawSession =
      session || this.marketCacheService.getCurrentMarketSession();
    const currentSession: 'market_open' | 'market_close' =
      rawSession === 'after_hours' ? 'market_close' : rawSession;
    this.logger.log(
      `🚀 Executing immediate update for session: ${currentSession}`,
    );
   ...
   */
  private async executeScheduledUpdate(
    session: 'market_open' | 'market_close',
  ): Promise<UpdateSummary> {
  ...
  private async updateFearGreedIndex(session: string): Promise<FearGreedIndex> {
    try {
      const fearGreedData =
        await this.fearGreedService.calculateFearGreedIndex();
      ...
      await this.marketCacheService.cacheMarketData({
        ...
        expiryTimestamp: new Date(
          Date.now() + this.CACHE_TTL.fearGreedIndex * 1000,
        ),
        dataQualityScore: fearGreedData.confidence,
      });

      this.logger.log(
        `📊 Fear & Greed Index updated: ${fearGreedData.value} (${fearGreedData.status})`,
      );
  ...
  private async updateEconomicIndicators(session: string): Promise<{
    ...
    try {
      const [interestRateResult, cpiResult, unemploymentResult] =
      ...
      const economicData = {
        interestRate:
          interestRateResult.status === 'fulfilled'
            ? interestRateResult.value
            : await this.fredService.getInterestRateData(),
        cpi:
          cpiResult.status === 'fulfilled'
            ? cpiResult.value
            : await this.fredService.getCPIData(),
        unemployment:
          unemploymentResult.status === 'fulfilled'
            ? unemploymentResult.value
            : await this.fredService.getUnemploymentData(),
      };
      ...
      await this.marketCacheService.cacheMarketData({
        ...
        apiSource: 'fred_api',
        expiryTimestamp: new Date(
          Date.now() + this.CACHE_TTL.economicIndicators * 1000,
        ),
      });

      this.logger.log(
        `📈 Economic indicators updated: Interest Rate ${economicData.interestRate?.value}%, CPI ${economicData.cpi?.value}, Unemployment ${economicData.unemployment?.value}%`,
      );
      return economicData;
    } catch (error) {
  ...
  private async updateSP500Sparkline(
    session: string,
  ): Promise<SP500SparklineData> {
    try {
      ...
      await this.marketCacheService.cacheMarketData({
        ...
        expiryTimestamp: new Date(
          Date.now() + this.CACHE_TTL.sp500Sparkline * 1000,
        ),
      });

      this.logger.log(
        `📊 S&P 500 sparkline updated: $${sp500Data.currentPrice} (${sp500Data.weeklyChange.toFixed(2)}%)`,
      );
      return sp500Data;
    } catch (error) {
  ...
  private async updateSectorPerformance(
    session: string,
  ): Promise<EnhancedSectorPerformance[]> {
    try {
      const sectorData =
        await this.alphaVantageService.getEnhancedSectorPerformance();
      ...
      await this.marketCacheService.cacheMarketData({
        ...
        apiSource: 'alpha_vantage',
        expiryTimestamp: new Date(
          Date.now() + this.CACHE_TTL.sectorPerformance * 1000,
        ),
      });

      this.logger.log(
        `🏭 Sector performance updated: ${sectorData.length} sectors`,
      );
      return sectorData;
    } catch (error) {
  ...
  private async updateVixData(session: string): Promise<VixData> {
    try {
      ...
      this.logger.log(
        `📊 VIX data updated: ${vixData.value} (${vixData.status})`,
      );
      return vixData;
    } catch (error) {
  ...
  async getUpdateStatus(): Promise<{
    ...
    try {
      ...
      let nextUpdate: Date;
      if (
        easternTime.getHours() < 9 ||
        (easternTime.getHours() === 9 && easternTime.getMinutes() < 30)
      ) {
        // Before market open
  ...
  private async updateSP500SparklineOnly(): Promise<SP500SparklineData> {
    try {
      ...
      this.logger.log(
        `⚡ SP500 intraday update: $${sp500Data.currentPrice} (${sp500Data.weeklyChange.toFixed(2)}%)`,
      );
      return sp500Data;
    } catch (error) {
  ...
  private async updateVixDataOnly(): Promise<VixData> {
    try {
      ...
      this.logger.log(
        `⚡ VIX intraday update: ${vixData.value} (${vixData.status})`,
      );
      return vixData;
    } catch (error) {
