...
export class AlphaVantageService {
  ...
  async getEnhancedSectorPerformance(): Promise<EnhancedSectorPerformance[]> {
    try {
      ...
      const sectorPromises = Object.entries(this.sectorETFs).map(
        async ([sectorName, symbol]) => {
          try {
            // Get current quote
            ...
            this.logger.warn(
              `Failed to fetch data for sector ${sectorName}:`,
              error.message,
            );
      ...
      this.logger.error(
        'Error fetching enhanced sector performance:',
        error.message,
      );
      return this.getMockEnhancedSectorData();
    }
  ...
  private processSectorData(
    ...
    try {
      ...
      const weeklyChange =
        ((currentWeekClose - lastWeekClose) / lastWeekClose) * 100;
      ...
      this.logger.warn(
        `Error processing sector data for ${sectorName}:`,
        error.message,
      );
      return this.getMockSectorData(sectorName, symbol);
    }
  ...
  private interpretVix(vix: number, change: number): string {
    ...
    return (
      interpretations[`${level}-${direction}`] ||
      'Market volatility monitoring required'
    );
  }
  ...
  private analyzeRotation(
    change: number,
    volume: number,
  ): 'rotating-in' | 'rotating-out' | 'stable' {
